...
dire <- "/home/itoledo/Dropbox/OnlineClass/Udacity/DataAnalysisR/Project3/example_at_work/Xa2300a_Xa8a/"
...
if (calamp$MEASUREMENT_ID != 0) {
  calamp$DATE_OBSERVED <- as.POSIXct(calamp$DATE_OBSERVED, 'GMT')
}
...
fieldsrc <- left_join(fieldsrc, select(scparam, paramRef, intTime, subScanDur), on='paramRef')
...
if (!"OBSERVE_TARGET" %in% calint$SCAN_INTENT) {
  ...
  cat(as.numeric(calint[calint$SCAN_INTENT == "OBSERVE_TARGET", 'scanNum']), "scans with intent OBSERVE_TARGET found.\\newline \n")
}

for (i in intents) {
  if (i %in% calint$SCAN_INTENT) {
    cat(as.numeric(calint[calint$SCAN_INTENT == i, 'scanNum']), "scans with intent", as.character(i), "found.\\newline \n")
  }
  else {
...
if (calamp$MEASUREMENT_ID == 0) {
  cat("A Solar System object was used. All OK.")
} else {
  ampsrc <- as.character(scan[scan$SCAN_INTENT == "CALIBRATE_AMPLI", "FIELD_NAME"])
  band <- as.character(sbsum$band)
  if (band == "ALMA_RB_03") {
    calsrc <- subset(calamp, FREQUENCY < 2e11)
    if (nrow(calsrc) > 0) {
      delt <- abs(difftime(calsrc$DATE_OBSERVED, slt$'Start Time:'))
      if (delt < difftime('2015-05-26', '2015-05-18', units="days")) {
        cat("Source has been observed within 8 days of the EB exec." , 
            "(latest obs on", as.character(calsrc$DATE_OBSERVED),") \\newline \n")
      } else {
        cat("Source HASN'T been observed within 8 days of the EB exec.",
            "(latest obs on", as.character(calsrc$DATE_OBSERVED),") \\newline \n")
        qaflag <- "Unset"
      }
    } else {
      cat("The grid source", ampsrc, 
          "has not been observed in Band 3 in more than 7 days \\newline \n")
      qaflat <- "Unset"
    }
  } else {
    calsrc3 <- subset(calamp, FREQUENCY < 2e11)
    calsrc7 <- subset(calamp, FREQUENCY > 3e11)
    delt3 <- abs(difftime(calsrc3$DATE_OBSERVED, slt$'Start Time:'))
    delt7 <- abs(difftime(calsrc7$DATE_OBSERVED, slt$'Start Time:'))
    if (delt3 < difftime('2015-05-26', '2015-05-18', units="days")) {
        cat("Source has been observed within 8 days of the EB exec. (Band 3)" , 
            "(latest obs on", as.character(calsrc3$DATE_OBSERVED),") \\newline \n")
      } else {
        cat("Source HASN'T been observed within 8 days of the EB exec. (Band 3)",
            "(latest obs on", as.character(calsrc3$DATE_OBSERVED),") \\newline \n")
        qaflag <- "Unset"
      }
    if (delt7 < difftime('2015-05-26', '2015-05-18', units="days")) {
        cat("Source has been observed within 8 days of the EB exec. (Band 7)" , 
            "(latest obs on", as.character(calsrc7$DATE_OBSERVED),") \\newline \n")
      } else {
        cat("Source HASN'T been observed within 8 days of the EB exec. (Band 7)",
            "(latest obs on", as.character(calsrc7$DATE_OBSERVED),") \\newline \n")
        qaflag <- "Unset"
      }
  }
...
for (f in field_tar) {
  ...
  if (fract - trunc(fract) != 0) {
    ...
    qaflag <- "SemiPass / Unset"
    cat("Target Name:", f, "\\newline \n")
    cat("Requested unique Pointings:", as.character(sb_src$pointings), 
        "\\newline \n")
    cat("Requested repetitions by pointing / Total pointings:",
        ceiling(sb_src$intTime / sb_src$subScanDur) / sb_src$pointings, 
        "/", 
        ceiling(sb_src$intTime / sb_src$subScanDur),
        "\\newline \n")
    cat("Number of executed Pointings:", nrow(field_sscan),
        "\\newline \\newline \n")
  }
}
cat("Recomendation:", "\\newline \n")
