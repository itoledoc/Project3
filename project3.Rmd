...
# by Frequency
ord <- as.character(
  arrange(data.frame(table(households$tph)),
          desc(Freq))$Var1)

ggplot(aes(factor(tph, levels = ord), y = 100 * ..count.. / sum(..count..)),
       data = households) + 
  geom_histogram(fill = "#67a9cf") + 
  theme(axis.text.x = element_text(angle = 40, hjust = 1)) +
...
houseinc <- households %>% 
  subset(person.id == 1 & !is.na(income.hh.av.rent)) %>% 
...
So, while the mean household income is US$`r summary(houseinc$income.hh.av.rent)[4]`
the median is at US$`r summary(houseinc$income.hh.av.rent)[3]`. How it does
compare with other countries?

```{r houseinc_summ, echo=F}
summary(houseinc$income.hh.av.rent)
```
...
ord <- as.character(
  arrange(data.frame(table(households$edependence)),
          desc(Freq))$Var1)
...
ggplot(aes(factor(edependence, levels = ord), y = 100 * ..count.. / sum(..count..)),
...
ggplot(aes(age, dep.work.income), data=subset(households, age > 0 & dep.work.income > 0)) +
...
ggplot(aes(income.hh.av, expenses.tot), data=subset(households, household.head == 1)) +
  geom_point(alpha=0.3) + scale_x_log10(breaks=c(100,1000,10000)) + geom_smooth()
```

```{r warning=F, echo=F, fig.align='center'}
p1 <- ggplot(aes(income.hh.av, income.hh.av - expenses.tot), data=subset(households, household.head == 1)) +
  geom_point(alpha=0.3) + scale_x_log10(breaks=c(100,1000,10000)) + geom_smooth() +
  coord_cartesian()

p2 <- ggplot(aes(income.hh.av, income.hh.av - expenses.tot), data=subset(households, household.head == 1)) +
  geom_point(alpha=0.3) + scale_x_log10(breaks=c(100,1000,10000)) + geom_smooth() +
...
```{r echo=F, fig.align='center'}
ggplot(aes(income.dec, income.hh.av - expenses.tot), data = subset(households, person.id == 1)) + geom_hline(yintercept=0) +
  geom_jitter(alpha=0.1) + geom_boxplot()

ggplot(aes(income.dec, expenses.tot / income.hh.av), data = subset(households, person.id == 1 & expenses.tot / income.hh.av <= 10)) + geom_hline(yintercept=1) +
...
tabexp <- expenses %>% group_by(GLOSA) %>% summarise(co = n())
usar1 <- as.character(as.data.frame(tabexp[order(-tabexp$co)[1:31][-5],])[,1])
moreexp <- subset(expenses, GLOSA %in% usar1)
...
tabexp3 <- moreexp %>% group_by(GLOSA) %>% summarise(med = median(GASTO))
usar3 <- as.character(as.data.frame(tabexp3[order(-tabexp3$med),])[,1])

ggplot(aes(factor(GLOSA, levels = usar3), GASTO), data = moreexp) + 
  geom_jitter(alpha = 0.3, color = "#67a9cf") + 
  geom_boxplot(alpha = 0.8) + 
...
ggplot(aes(age.group), data = subset(households, !is.na(dep.work.income) & !is.na(age.group))) + geom_boxplot(aes(y=dep.work.income, fill = gender)) + scale_y_continuous(trans='log1p') + scale_fill_brewer(palette = "Paired")



t1 <- dcast(subset(households, !is.na(dep.work.income)), age.group ~ gender, fun = median, value.var = 'dep.work.income')

w <- households %>% subset(!is.na(dep.work.income)) %>% group_by(age.group, gender) %>% summarise(inco = median(dep.work.income))

ggplot(aes(age.group), data = subset(w, !is.na(age.group))) + geom_histogram(aes(y = inco, fill = gender), stat = 'identity', alpha=0.5, position = "identity", color = 'grey') + scale_fill_brewer(palette = "Paired")
