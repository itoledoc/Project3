...
ggplot(aes(factor(kinship, levels = ord),
           y = ..count..),
       data = households) + 
  geom_histogram(fill = "#67a9cf") + 
  theme(axis.text.x=element_text(angle = 40, hjust = 1)) +
...
ggplot(aes(factor(tph, levels = ord),
...
t <- summary(houseinc$income.hh.av.rent)
pander(data.frame(Statistic = names(t), Value = as.vector(t)))
```
...
pandoc.table(
  data.frame(Percentil = names(decil), Value = as.vector(decil)),
...
```{r checkincquant_plot, echo=F, fig.align='center'}
...
```{r inhab_summ, echo=F}
t <- summary(inhab$num.inhabitants)
pander(data.frame(Statistic = names(t), Value = as.vector(t)))
```
...
```{r age_inc, echo=F, fig.align='center'}
...
```{r inc_exp2, warning=F, echo=F, fig.align='center', message=F}
